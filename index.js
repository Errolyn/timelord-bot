...
// Prevents on guildMemberAdd event from triggering an
// action if a code of conduct channel is not available.
const cocChannel = process.env.COC_CHANNEL_ID;
if (cocChannel) {
  bot.on('guildMemberAdd', async (guild, member) => {
    const userName = member.username;
    const serverName = guild.name;
    const channel = await bot.getDMChannel(member.id); // Finds the users direct message id
...
bot.registerCommand(
  ...
  (msg) => {
    ...
    const content = `${userName} posted in ${messageChannelName}: \n${contentForNewsChannel} \n${attachments}`;
...
function rollDecider(command) {
  ...
  commandCharacters.forEach(character => {
    const tempIndex = commandArray.indexOf(character);
    if (tempIndex > 0) {
